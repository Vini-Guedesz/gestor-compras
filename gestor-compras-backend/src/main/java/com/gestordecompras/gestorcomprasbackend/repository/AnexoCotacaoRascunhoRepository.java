...
/**
 * Repository para gerenciar anexos de cotações de rascunho
 * Suporta deduplificação através de busca por hash SHA-256
 */
@Repository
public interface AnexoCotacaoRascunhoRepository extends JpaRepository<AnexoCotacaoRascunho, Long> {
     ...
     */
    Optional<AnexoCotacaoRascunho> findFirstByHashSha256(String hashSha256);
     ...
     */
    List<AnexoCotacaoRascunho> findAllByHashSha256(String hashSha256);
     ...
     */
    @Query("SELECT COUNT(DISTINCT a.hashSha256) FROM AnexoCotacaoRascunho a WHERE a.hashSha256 IS NOT NULL")
     ...
     */
    @Query("SELECT a.hashSha256, COUNT(a) as count FROM AnexoCotacaoRascunho a " +
